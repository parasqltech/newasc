											...
											{tags && tags.edges.map( (propd,i) => {
															return (
																...
																(propd.node.name !== 'Uncategorised') ? (
																	<><Link to={"/tags/"+propd.node.slug+"/"} className="mr-1 color-70 font-regular decoration-none">{propd.node.name}</Link> <p className="mb-0 color-70 font-regular">{propd.node.count}</p></>
																) : ("") 
															)
														})}
